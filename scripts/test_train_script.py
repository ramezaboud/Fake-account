...
def test_train_script_creates_pipeline(tmp_path):
    ...
    `data/users.csv` and `data/fusers.csv`, provides a stubbed
    `sklearn.model_selection.RandomizedSearchCV` implementation to avoid long
    ...
    # create small data folder with minimal rows
    data_dir = project_tmp / 'data'
    data_dir.mkdir()
    users_csv = data_dir / 'users.csv'
    fusers_csv = data_dir / 'fusers.csv'

    header = (
        'lang,created_at,statuses_count,followers_count,friends_count,'
        'favourites_count,listed_count,name,description,default_profile,verified\n'
    )
    # two small rows for each file
    users_csv.write_text(
        header +
        'en,2020-01-01 00:00:00,100,50,20,5,1,John Doe,hello,0,0\n'
        'fr,2019-06-15 12:34:56,200,150,40,10,2,Jane Roe,hi,0,1\n'
    )
    fusers_csv.write_text(
        header +
        'es,2018-03-03 03:03:03,10,5,2,0,0,Faux One,fake,1,0\n'
        'pt,2017-07-07 07:07:07,5,2,1,0,0,Faux Two,bot,1,0\n'
    )
    ...
    stub_code = '''
def train_test_split(X, y, test_size=0.4, random_state=None, stratify=None):
...
class RandomizedSearchCV:
    def __init__(self, estimator, param_distributions=None, n_iter=1, cv=2,
                 n_jobs=None, scoring=None, random_state=None, verbose=0):
...
'''
    ...
    train_text = train_text.replace(
        ...
        'from sklearn_model_selection_stub import train_test_split, RandomizedSearchCV'
    )
